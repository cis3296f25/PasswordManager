...
def get_credential(site):
    response = requests.get(f"{BASE_URL}/credentials/{site}")
    return response.json()

#calls GET generate-password
def get_new_generated_password():
    response = requests.get(f"{BASE_URL}/credentials/generate-password")
    return response.json()

#calls DELETE
def delete_credential(site):
    response = requests.delete(f"{BASE_URL}/credentials/{site}")
    return response.json()
...
if __name__ == "__main__":
    ...
    test_pass = test_pass["generatedPassword"]
